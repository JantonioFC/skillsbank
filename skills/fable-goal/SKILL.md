---
name: fable-goal
description: "Convert a rambling description of a desired outcome into one polished /goal prompt for a fresh session. Use when the user says '/fable-goal', 'turn this into a goal prompt', or rambles about something they want made and asks for the prompt that builds it. Outputs the prompt, not the build."
risk: safe
source: community
...
Use this skill when the user wants the *prompt* that will build something in a fresh session, not the build itself. Trigger on `/fable-goal`, 'turn this into a goal prompt', or a rambling outcome that needs to become one clean, paste-ready `/goal` prompt. Do not use when they want it built right now.
