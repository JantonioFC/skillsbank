...
Use this skill when the user runs `/si:memory-status` or asks how full, healthy, or bloated the agent's auto-memory is — line counts, topic files, capacity, and stale entries. It only reports; it does not modify memory.
